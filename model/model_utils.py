...
class ProteinMPNN(nn.Module):
    def __init__(
        ...
        vocab=21,
        k_neighbors=32,  # Modified from 48 to 32
        augment_eps=0.0,
        dropout=0.0,
    ):
        super().__init__()
    ...
    def encode(self, feature_dict):
        X_1 = feature_dict["X_1"]
        X_2 = feature_dict["X_2"]
        ...
        E_1, E_idx_1 = self.features(feature_dict_1)
        E_2, E_idx_2 = self.features(feature_dict_2)
        E = torch.cat([E_1, E_2], 2)
        E_idx = torch.cat([E_idx_1, E_idx_2], 2)
        h_V = torch.zeros(
    ...
    def forward(self, feature_dict):
        ...
        chain_mask *= mask  # update chain_mask to include missing regions
        decoding_order = torch.argsort(
            (chain_mask + 0.0001)
            * (torch.abs(torch.randn(chain_mask.shape, device=device)))
        )  # numbers will be smaller for places where chain_mask = 0.0 and higher for places where chain_mask = 1.0
...
class ProteinFeatures(nn.Module):
    ...
    def __init__(
        ...
        num_rbf=16,
        top_k=32,  # Modified from 48 to 32
        augment_eps=0.0,
    ):
        ...
        edge_in = num_positional_embeddings + num_rbf * 25  # 416
        self.edge_embedding = nn.Linear(edge_in, edge_features, bias=False)
        self.norm_edges = nn.LayerNorm(edge_features)

    def _dist(self, X, mask, eps=1e-6):
        mask_2D = torch.unsqueeze(mask, 1) * torch.unsqueeze(mask, 2)
        dX = torch.unsqueeze(X, 1) - torch.unsqueeze(X, 2)  # [B, L, L, 3]
        D = mask_2D * torch.sqrt(torch.sum(dX**2, 3) + eps)  # [B, L, L]
        D_max, _ = torch.max(D, -1, keepdim=True)  # [B, L, 1]
        D_adjust = D + (1.0 - mask_2D) * D_max  # [B, L, L]
    ...
    def forward(self, feature_dict):
        ...
        RBF_all = torch.cat(RBF_all, dim=-1)  # [B, L, K, 16 * 25]
        ...
        E_chains = gather_edges(d_chains[:, :, :, None], E_idx)[:, :, :, 0]  # [B, L, K]
        E_positional = self.embeddings(offset.long(), E_chains)  # [B, L, K, 16]

        E = torch.cat((E_positional, RBF_all), -1)  # [B, L, K, 416]
        E = self.edge_embedding(E)  # [B, L, K, 128]
        E = self.norm_edges(E)  # [B, L, K, 128]
...
class PositionWiseFeedForward(nn.Module):
    ...
    def forward(self, h_V):
        h = self.W_out(self.act(self.W_in(h_V)))
        return h
